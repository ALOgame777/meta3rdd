...
public class ChatManager : MonoBehaviourPun
{
    ...
    // 채팅이 추가되기 전에 Content의 높이(H) 값을 가지고 있는 함수
    float prevContentH;
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            {
                // 만약에 ui가 클릭이 되지 않았다면
                if (EventSystem.current.IsPointerOverGameObject() == false) // 마우스 포인터가 ui위에 있으면 true, false
                {
                    // 마우스 포인터를 비활성화
    ...
    void AddChat(string chat)
    {
        ...
        // 가져온 컴포넌트의 onAutoScroll 변수에 onAutoScrollBottom을 설정
    ...
    {
        // Chatview의 H 보다 Content 의 H 값이 크다면( 스크롤이 가능한 상태라면)
